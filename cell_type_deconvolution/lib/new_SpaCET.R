# # Use a specific Python environment (replace with your environment path)
...
.libPaths(c('/vf/users/shulmaned/R/4.3/library',
                 '/vf/users/shulmaned/R/rhel8/4.3',
                 '/usr/local/apps/R/4.3/site-library_4.3.2',
                 '/usr/local/apps/R/4.3/4.3.2/lib64/R/library'))

# +
#library(SpaCET, lib.loc = "/vf/users/shulmaned/R/4.3/library/")

# +
SpaCET.deconvolution_new_cpm <- function(st.matrix.data, cancerType, adjacentNormal=FALSE, coreNo=6)
{
  ...
  if(adjacentNormal==TRUE)
  {
    ...
    malRes <- inferMal_cor_cpm(st.matrix.data,cancerType)
  }
...
# +
SpaCET.deconvolution_new <- function(st.matrix.data, cancerType, adjacentNormal=FALSE, coreNo=6)
{
  coreNoDect <- parallel::detectCores(logical = FALSE)
  if(coreNoDect<coreNo)
  {
...
# +
inferMal_cor_cpm <- function(st.matrix.data, cancerType)
{
  load( system.file("extdata", 'cancerDictionary.rda', package = 'SpaCET') )
  cancerTypes <- unique(c(names(cancerDictionary$CNA),names(cancerDictionary$expr)))
  cancerTypes <- sapply(strsplit(cancerTypes,"_",fixed=T),function(x) return(x[2]))
